...
-- sets {1, 2} and {a, b} is { (1, a), (1, b), (2, a), (2, b) }.
-- set_product: (s1: set a) -> (s2: set b) -> set (pair a b)
pair_all: a -> list b -> list (pair a b)
pair_all v nil = nil
pair_all v (h::t) = (mkPair v h) :: (pair_all v t)

list_pair: (list a) -> (list b) -> (list (pair a b))
list_pair la nil = nil
list_pair nil _ = nil
list_pair (h::t) lb = ((pair_all h lb) ++ (list_pair t lb))

set_product (mkSet la) (mkSet lb) = mkSet (list_pair la lb)
...
set_toString (mkSet a) = listString a
-------------------------------------
